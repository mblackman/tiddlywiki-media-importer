...
(function() {
  ...
  function formatCustomDate() {
    ...
    var timePart = hours + ':' + String(d.getMinutes()).padStart(2, '0') + ' ' + ampm;
  ...
  function registerMediaImporter(config) {
    ...
    var searchPrefix = '$:/temp/search/' + (config.prefix || type + '-result') + '/';
    ...
    $tw.rootWidget.addEventListener('tm-search-' + type, function(event) {
      ...
      if (config.apiKeyTiddler) {
        ...
        if (!apiKey) {
          alert(
            'Missing API Key for ' +
              type +
              '. Please set it in the Media Importer Plugin Config.',
          );
          return;
        }
      }
      ...
      for (var index = 0; index < oldResults.length; index++) {
    ...
    $tw.rootWidget.addEventListener('tm-fetch-' + type, function(event) {
      var temporaryTitle = event.param;
      var temporaryTiddler = $tw.wiki.getTiddler(temporaryTitle);
  ...
  exports.startup = function() {
    ...
    registerMediaImporter({
      ...
      searchFn: function(query, apiKey, page) {
        var p = page || 1;
        var url = 'https://openlibrary.org/search.json?limit=5&q=' +
            ...
            return data.docs.slice(0, 5).map(book => {
              var coverUrl = book.cover_i
                ? 'https://covers.openlibrary.org/b/id/' +
                  ...
                  '-L.jpg'
                : '';
              var fetchId = book.isbn && book.isbn.length > 0
                ? book.isbn[0]
                : book.key || '';
      ...
      fetchFn: function(draftFields) {
        ...
        var url = id.indexOf('/works/') === 0 || id.indexOf('/books/') === 0
        ...
        return fetch(url)
          ...
          .then(data => {
            var cleanTitle = typeof data.title === 'object' && data.title.value
              ? data.title.value
              : data.title;
    ...
    registerMediaImporter({
      ...
      searchFn: function(query, apiKey, page) {
        var p = page || 1;
        var url = 'https://comicvine.gamespot.com/api/search/?api_key=' +
          apiKey +
